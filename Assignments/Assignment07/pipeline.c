...
int main(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; ++i) {
        // Creating pipeline to next program (if there is one)., assigning read_desc and write_desc
        int read_desc;
        int write_desc;
        if (i < argc - 1){
            int pipe_dsc[2];
            ASSERT_SYS_OK(pipe(pipe_dsc));
            read_desc = pipe_dsc[0];
            write_desc = pipe_dsc[1];
        }
        ...
        if (!pid) {
            ...
            if (i > 1) {
                ...
                ASSERT_SYS_OK(close(prev_read_dsc));  // Close the original copy.
            }
            // For all programs but the last, replace stdout with the current pipe's writing end.
            if (i < argc - 1) {
                ASSERT_SYS_OK(close(read_desc));  // Closing reading end of pipe

                fprintf(stderr, "Child %d: replacing stdout descriptor %d with a copy of %d\n", i, STDOUT_FILENO, write_desc);

                ASSERT_SYS_OK(dup2(write_desc, STDOUT_FILENO));
                ASSERT_SYS_OK(close(write_desc)); 
            }
